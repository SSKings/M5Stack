...
LGFX_Sprite GameSprite = LGFX_Sprite(&M5.Lcd);
...
struct GameSys
{
    ...
    uint16_t BK_color;
} sys = {0, 0, 0, false, false, false, nullptr, nullptr, {nullptr, nullptr, nullptr}, 0, 0, 0, 0, 0, 0, WHITE};
...
void drawRing(uint64_t index)
{
    ...
    for (size_t y = 0; y < 12; y++)
    {
        for (size_t x = 0; x < 240; x++)
        {
            ...
            if (color_index != 0x00)
                GameSprite.drawPixel(x, 105 + y, Gray16[color_index]);
        }
    }
}
...
void setup()
{
    ...
    GameSprite.createSprite(240, 135); // Alterado para 240x135
    GameSprite.fillRect(0, 0, 240, 135, WHITE); // Alterado para 240x135
...
void loop()
{
    ...
    else if (sys.dinostate == DINO_OVER)
    {
        ...
        if (M5.BtnA.wasPressed() || M5.BtnB.wasPressed())
        {
            GameSprite.fillRect(0, 0, 240, 135, sys.BK_color); // Alterado para 240x135
            sys.systick = 0;
            sys.speed = 2;
            sys.dinostate = DINO_RUN;
